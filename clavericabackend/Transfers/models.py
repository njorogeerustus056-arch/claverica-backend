...
class Transfer(models.Model):
    ...
    def __str__(self):
        return f"{self.transfer_id} - {self.sender.username} → {self.recipient_name or 'Unknown'} - {self.amount} {self.currency}"
...
class TACCode(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='tac_codes')
    ...
    def __str__(self):
        return f"TAC-{self.code} for {self.user.username}"
