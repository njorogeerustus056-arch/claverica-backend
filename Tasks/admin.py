...
class TaskAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('title', 'description', 'task_type', 'status')
        }),
        ('Reward Details', {
            'fields': ('reward_amount', 'currency')
        }),
        ('Task Settings', {
            'fields': (
...
class UserTaskAdmin(admin.ModelAdmin):
    list_display = [
        'user', 'task', 'status', 'reward_earned', 
        'reward_paid', 'started_at', 'completed_at'
    ]
    ...
    fieldsets = (
        ...
        ('Submission', {
            'fields': ('submission_data', 'submission_notes', 'submitted_at')
        }),
        ('Reward', {
            'fields': ('reward_earned', 'reward_paid', 'reward_paid_at')
        }),
        ('Review', {
...
class RewardWithdrawalAdmin(admin.ModelAdmin):
    list_display = [
        'user', 'amount', 'currency', 'withdrawal_method', 
    ...
    fieldsets = (
        ('Withdrawal Information', {
            'fields': ('user', 'amount', 'currency', 'withdrawal_method')
        }),
        ('Account Details', {
            'fields': ('account_details',)
        }),
        ('Status & Processing', {
            'fields': ('status', 'transaction_id', 'processor_notes')
        }),
        ('Timestamps', {
...
class UserRewardBalanceAdmin(admin.ModelAdmin):
    list_display = [
        ...
        'total_earned', 'total_withdrawn', 'tasks_completed'
    ]
    ...
    fieldsets = (
        ...
        ('Balance', {
            'fields': ('available_balance', 'pending_balance', 'total_earned', 'total_withdrawn', 'currency')
        }),
        ('Statistics', {
            'fields': ('tasks_completed', 'tasks_pending')
        }),
        ('Timestamps', {
